...
def breply(request, bno):
  if request.method == "GET":
    ...
    bgroup = int(request.POST.get("bgroup"))  # 답글들 묶음(그룹핑)
    ...
    # 같은 bgroup속한 bstep더 큰것만 검색해서, 데이터 1씩 증가
...
def bview(request, bno):
  ...
  # filter() 형태 - update() 명렁어가 존재함.
  # F함수 - 필드 값을 참조
  qs = Board.objects.filter(bno=bno)
  # 이전글
  # prev_qs = Board.objects.filter().order_by("-bgroup","bstep").first()
  # 다음글
  # next_qs = Board.objects.filter().order_by("group","-bstep").first()
  context = {"board":qs[0]}
